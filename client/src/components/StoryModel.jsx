...
const StoryModel = ({setShowModal, fetchStories}) => {
    ...
    const handleMediaUpload = (e) => {
        ...
        if(file){
            setMedia(file)
            setPreviewUrl(URL.createObjectURL(file))  // static method in javascript
        }
    }
    ...
    <div className='fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4'>
        <div className='w-full max-w-md'>
            ...
            <div className='rounded-lg h-96 flex items-center justify-center relative' style={{backgroundColor: background}}>
                ...
                {mode === 'text' && (
                    <textarea name='' id='' className='bg-transparent text-white w-full h-full p-6 text-lg resize-none focus:outline-none' placeholder="What's in your mind ? " onChange={(e) => setText(e.target.value)} value={text}/>
                )}
            ...
            <div className='flex gap-2 mt-4'>
                ...
                <label onClick={() => setMode('media')} className={`flex-1 flex items-center justify-center gap-2 p-2 rounded cursor-pointer ${mode === 'media' ? 'bg-white text-black' : 'bg-zinc-800'}`}>
